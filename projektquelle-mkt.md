# Projektquelle — Marketingabteilung

> **Wahrheits-Kanal dieses Projekts.** Änderungen nur durch die
> Leitsession beim Debrief, per Anker-Edit (nie Vollumschrift), immer im
> Vier-Stellen-Sync: (1) Changelog-Header unten ergänzen, (2) betroffenen
> Abschnitt anpassen, (3) Nächster-Schritt-Block nachziehen,
> (4) STATUS.md aktualisieren (inkl. 2-Runden-Rotation ins Archiv).
...
(Heads → Teamleads → Worker mit Freigabeschlaufen) ist ZIELBILD und
liegt als geparkte Idee in § 5 — technisch wird flach gestartet.

Das Unternehmen digitalisiert inhabergeführte Häuser komplett entlang der
Guest Journey (PMS-Einführung/Apaleo, Terminal-Check-in Touch+Sprache,
Pre-Check-in, Meldeschein, Payment, Zutritt, KI-Gästekommunikation) —
...
*(Je Strang: Code · Ziel · Arbeitspakete mit Fertig-Kriterium · Status.
Arbeitspakete sind disjunkt; Status-Einträge trennen Bau ≠ Beleg.
Strang-Übersichtstabelle: CLAUDE.md „Projektstruktur".)*
...
- **MARKE3** Sicherung: Domains (defensiv .de/.com-Varianten), Handles,
  Markenanmeldung (Klassen, Inhaberschaft bei GbR → Zentrale/StB).
...
- **BELEG2** Referenzvereinbarungen aller 3 Pilotobjekte (Namensnennung,
  ...
  Zulieferung an Zentrale. Fertig: 3 unterschriebene Vereinbarungen.
- **BELEG3** Demo-Video Voice-Check-in (60–90 s, echtes Gerät,
...
- **HANDEL3** Kanal-Rechtsmatrix (UWG § 7: Kalt-E-Mail vs. Telefon vs.
  Brief vs. LinkedIn; Signatur-Pflichtangaben) — VOR dem Bau der
  Zielkundenliste. Fertig: Matrix mit Quellen.
- **HANDEL4** Lead-Prozess: Reaktionsfrist, Discovery-Leitfaden,
  Kapazitätsdeckel (Leads/Woche des Solo-Gründers), minimales
  Lead-Tracking. Fertig: Prozess dokumentiert + Tracking läuft.
- **HANDEL5** Vertrags-/AVV-Anforderungen aus Marketing-Sicht
  (Referenzklausel, AVV mit Häusern) als Zulieferung → Postkorb Zentrale.

### MKT-WEB — Website & digitale Präsenz (startet nach Namensentscheid)
- **WEB1** Konzept + Informationsarchitektur + Content (vollwertig,
  echte Fotos aus den Pilotobjekten mit Freigaben — nie nackte
  Landingpage). Fertig: freigegebenes Konzept + Texte.
- **WEB2** Bau + Deploy (Cloudflare), SEO-Grundlagen (inkl.
  Regulatorik-Content-Kandidaten: Meldeschein-Pflicht, BFSG),
  datenschutzkonformes Tracking (Werkzeug-Entscheid), Google Business
  Profile. Fertig: live unter neuer Domain + Redirects.
- **WEB3** Rechtspflichten: Impressum, Datenschutz (inkl. VVT-Eintrag),
  Cookie-Lösung, BFSG-Einstufung mit Beleg, Bildrechte-Register.
...
- **AKQ1** Kanalmix-Entscheid NACH FUND2 (Kandidaten: Founder-
  Direktvertrieb, Referenz-/Empfehlungsmechanik, Apaleo-Ökosystem/
...
- **AKQ4** Erster zahlender Kunde. Fertig: unterschriebener Vertrag —
  **Zielmarke Mitte November 2026.**

### MKT-OPS — AI-native Betrieb & Messung (flach)
- **OPS1** Lead-Tracking + Funnel-KPIs (Erstgespräche/Woche · Angebote/
...
| System | Wofür | Zugangsweg je Handlungsart | Exklusiv |
|---|---|---|---|
| Cloudflare | Domains, Website (Pages/Worker), DNS | API/Dashboard; Deploy per Wrangler; User-Klick für Account-Aktionen | ja |
| Google Workspace/Gmail | Geschäfts-Mail, Entwürfe, Recherche | Gmail-MCP (Session-Pool!); Admin-Aktionen = User-Klick | ja |
| Domain-Registrar(e) | Bestand unklar — **Inventur offen** | Dashboard mit User | ja |
...
- **09.08.2026 (E1) Rebrand kommt.** Wortlaut (Voice 09.08.): „Deswegen
  ...
  (MARKE5), Markenrecherche beider Namen (MARKE1), Sicherung vor
  Ankündigung (MARKE3), Gesellschafter-Freigabe (MARKE2). **Technischer
  Träger des Terminrisikos:** Postkorb-Meldung an Zentrale
  (GbR-Namensführung vor Gewerbeanmeldung ~Mitte August).
- **09.08.2026 (E2) Zielgruppe ausdrücklich auch größere Häuser** —
  „steht nicht zur Diskussion"; USP/Portfolio werden dafür in FUND neu
  ausgearbeitet. Die 58-jährige-Wirte-These der Prüfer wird dadurch
  selbst zur Hypothese → FUND2 Persona-Validierung entscheidet den
  Kanalmix, nicht die Prüfer-Annahme.
...
- **Wellness-Proximity-Push** (geparkt 09.08.2026; User-Gedankenspiel als
  ...
  Wellnessbereich vorbei, löst ein Näherungssensor (Beacon/„Bewegungs-
  melder an der Steckdose") eine gezielte Push-Benachrichtigung mit
  Angebot aus. **Zielort: Produkt (heypensio-Repo), Übergabe über dessen
  Leitsession** — hier nur als FUND1-Input (Portfolio-Illustration).
  Prämissen beim Ent-Parken prüfen: DSGVO/Tracking-Einwilligung,
  ePrivacy, Hardware-Frage.
- **Newsletter als Betriebskanal** (geparkt 09.08.2026; Prämisse: Liste
...
09.08.2026: Projekt aufgesetzt per `/projekt-init` (Gerüst-Stand siehe
CLAUDE.md-Kopf). Planfassung nach dreifacher Prüfrunde (Prämissen-Kritik ·
