# R18-B — Zitatwächter: Präfix-Reparatur und Inventar der 36 NEU

- **Rolle:** Codex, Strang B
- **Datum:** 17.08.2026
- **Messobjekt:** Git-Archiv des festgeschriebenen Stands `86afceb`
- **Rohbeleg-Pool:** `sensibel/` aus dem Live-Repo, nur als ausdrücklich vorgegebener lokaler Rohbeleg-Pool
- **Commit/Push:** keiner; dieser Strang commitet und pusht nicht
- **Token-Verbrauch:** in dieser Oberfläche nicht verlässlich erhebbar; keine Subagenten eingesetzt

## 1. Ergebnis in einem Satz

Der Fehler bei verschachtelten Markdown-Zitatpräfixen ist mit einer zuerst roten und danach grünen Regression repariert, die eingefrorenen Nenner bleiben exakt bei R15 `507/863`, R16 `43/43` und R17 `442/447`, alle 36 ursprünglichen NEU-Fundstellen sind disjunkt inventarisiert, sieben ausdrücklich vorgegebene Eigenformulierungen sind begründet in `EIGEN_R18` aufgenommen, und der produktive Restnenner beträgt exakt **`36 - 0 α - 7 EIGEN_R18 = 29 NEU`**.

## 2. Messaufbau und Abgrenzung

Der Arbeitsbaum wurde **nicht** als Prüfobjekt verwendet. Aus `86afceb` wurde per `git archive` ein Export außerhalb des Repos erzeugt:

`C:\Users\qwafa_2dwxzia\firma\r18b-scratch-86afceb`

Alle produktiven Läufe erhielten ausdrücklich:

```text
--pruefwurzel=C:\Users\qwafa_2dwxzia\firma\r18b-scratch-86afceb
--pruefstand=86afceb
--rohbelege=C:\Users\qwafa_2dwxzia\firma\marketing\sensibel
```

Die zu liefernde Skriptfassung wurde am Ende aus dem Live-Repo auf genau diesen Export angewandt. Die 27 Zieldateien und zwölf versionierten Pool-Dateien stammen damit ausschließlich aus dem eingefrorenen Export; nur der git-ignorierte Rohbeleg-Pool stammt, wie beauftragt, aus dem Live-Repo.

Geändert wurden nur:

- `akquise/pruefe-zitate.js`
- `akquise/selbsttest-zitate.md`
- `protokolle/R18-B-bericht.md`

`akquise/zitat-baseline-2026-08-17.md` brauchte keine Nennerzeile und blieb unverändert. `ROHBELEG_OFFEN_R17` blieb in Inhalt und Umfang unverändert.

## 3. Reparatur der Normalisierung

### 3.1 Fehlerursache

Die bisherige Regel in `norm()` entfernte mit `^[ \t]*>[ \t]?` pro Zeile nur **ein** `>`-Präfix. Ein mittig in einem extrahierten Zitat stehendes `> >` hinterließ deshalb ein weiteres `>` im normalisierten Wortlaut. `normRohbeleg()` kannte die Wiederholung bereits, der versionierte Ziel-/Pool-Abgleich aber nicht.

Die neue Regel `^[ \t]*(?:>[ \t]?)+` entfernt ein oder mehrere Präfixglieder. Sie läuft in derselben `norm()`-Funktion für Ziel und versionierte Quelle und ist deshalb symmetrisch.

### 3.2 Reihenfolge

Das Präfix wird **vor** der Dehyphenierung entfernt. Nur so wird beispielsweise
...
zu `Silben-trennung`. Bei umgekehrter Reihenfolge verhindert das noch vorhandene `>` die Regel `-\s+`; genau diese falsche Reihenfolge wird im Selbsttest als Gegenprobe verworfen.

### 3.3 Eingefrorene Baseline bleibt eingefroren

Eine unbedingte Neunormalisierung aller alten Fundstellen würde neben den 36 NEU auch bereits festgeschriebene Pool-, Rohbeleg- oder Baseline-Zuordnungen neu bewerten. Deshalb reproduziert der Produktivpfad zunächst mit `normLegacy()` den Status, mit dem die Baseline entstanden ist. **Nur Fundstellen, die unter dieser Legacy-Fassung NEU waren**, gelangen in den reparierten symmetrischen Vergleich. Dadurch behebt R18-B den konkreten Wächterfehler, ohne den historischen Nenner still umzuschreiben.

Der alte `EIGEN_R17`-Wortlaut mit den versehentlich verbliebenen Zeichen `> Briefpost` und `> Art.` wurde auf den tatsächlichen Wortlaut ohne Präfixreste gebracht. Ergebnis: keine verwaiste EIGEN-Ausnahme; `35/35` Einträge vorhanden und `35/35` begründet.

## 4. Regressionsbeleg: vor dem Fix rot, nach dem Fix grün

Die neuen Muster prüfen beide Richtungen, die Stufenfolge und eine echte Gegenprobe:

- Präfix nur im Ziel, Quelle ohne Präfix: muss treffen.
- Präfix nur in der Quelle, Ziel ohne Präfix und mit Silbentrennung: muss treffen.
- Verfälschtes Wort trotz gleicher Präfixstruktur: muss **NEU** bleiben.
- Dehyphenierung vor Präfixentfernung: muss als falsche Reihenfolge verworfen werden.
- sieben `EIGEN_R18` jeweils positiv und mit je einer Wortlautabweichung negativ.
- reparierter `EIGEN_R17`-Eintrag positiv und verfälschte Gegenrichtung negativ.

**Lauf vor der Änderung von `norm()`**, aber bereits mit den neuen Tests: Exit `1`, `Muster (j) 0/2`, `Praefix vor Dehyphenierung: NEIN`, `EIGEN_R17 Praefix 0/1`, `SELBSTTEST FEHLGESCHLAGEN`.

**Lauf nach der Änderung:** Exit `0`:

```text
=== SELBSTTEST: dieselben Funktionen wie der Produktivpfad ===
Zitate extrahiert                 : 46 / 46
Muster (a) NEU                   : 9 / 9
...
Muster (j) > > symmetrisch       : 2 / 2 bestaetigt
Muster (j) TREFFEN-NICHT         : 1 / 1 NEU
Praefix vor Dehyphenierung       : JA
Dehyphenierung davor verworfen   : JA
Muster (k) EIGEN_R18 positiv     : 7 / 7
Muster (l) EIGEN_R18 negativ     : 7 / 7 NEU
Muster (m) EIGEN_R17 Praefix     : 1 / 1 EIGEN
Muster (m) Gegenrichtung         : 1 / 1 NEU
Selbstausschluss ZIEL            : 27 / 27
LF/CRLF invariant                : JA
SELBSTTEST BESTANDEN
```

Damit sind die bereits vorhandenen 27 Selbstausschluss-Prüfungen weiterhin grün; die neuen Zeilen prüfen die beiden Präfixrichtungen zusätzlich.

## 5. Klassensystem und Summenprobe

| Klasse | Bedeutung in diesem Inventar | Anzahl |
|---|---|---:|
| α | nach reparierter Normalisierung durch versionierten oder Rohbeleg-Pool bestätigt | 0 |
| β | eigene Formulierung, Selbstzitat oder Warn-/Überschriftsformel; Ursprung per `git log -S` geprüft | 8 |
| γ | fremdes Zitat ohne Treffer im **vorliegenden** Rohbeleg-Pool | 7 |
| δ | byte-identische Ziel-zu-Ziel-Quelle, die nicht im POOL liegt | 2 |
| ε | zweite Fundstelle eines bereits einmal verbrauchten Baseline-Texts | 1 |
| ζ | zitierte Vorfassung in einem Korrektur-/Reparaturkontext | 18 |
| **Summe** | **α + β + γ + δ + ε + ζ** | **36** |

α ist null: Keiner der 36 ursprünglichen NEU-Texte erhält allein durch die neue Präfixnormalisierung einen Treffer im versionierten oder lokalen Rohbeleg-Pool. Der Fehler ist dennoch real und durch die isolierte Alt/Neu-Regression belegt; die betroffenen realen Fundstellen sind hier Eigen-/Reparaturtexte oder nicht byte-identische Verdichtungen.

## 6. Vollinventar der 36 ursprünglichen NEU

Evidenzkürzel: `R` = sichtbarer Reparatur-/Vorfassungsvermerk am Prüfstand; `S` = `git log -S`; `P` = produktiver Pool-/Rohbeleg-Abgleich; `T` = byte-identische andere Zieldatei; `B` = zweite Baseline-Fundstelle. Die Spalte „parallel in Reparatur A“ bezeichnet nur den belegten direkten Arbeitsüberschnitt, nicht eine Vermutung.

| Nr. | Fundstelle | Text | Klasse | Evidenz / Herkunft | parallel in Reparatur A |
|---:|---|---|:---:|---|:---:|
| 1 | `akquise/akquiseplan.md:413` | `diese Datei ist noch NICHT scharf zu stellen` | δ | T: byte-identisch in `akquise/sperrdatei-struktur.md:42`; Zieldatei liegt nicht im POOL | — |
| 2 | `akquise/akquiseplan.md:413` | `es darf noch keine Welle geben` | δ | T: byte-identisch in `akquise/wellenprotokoll-vorlage.md:31`; Zieldatei liegt nicht im POOL | — |
| 3 | `akquise/interessenabwaegung-o8.md:654` | `das LG stützt sich auf § 3a UWG` | β | Warnformel für eine ausdrücklich verkürzte Lesart; S: Ursprung `af92adb` | — |
| 4 | `akquise/interessenabwaegung-o8.md:676` | `und sie ist die einzige, deren Tatbestand B2B einschließt` | ζ | R: ausdrücklich als widerlegte Erstfassung des Nachtrags ausgewiesen | — |
| 5 | `akquise/interessenabwaegung-o8.md:1229` | `über § 3a UWG bzw. § 3 Abs. 2 UWG` | ζ | R: Nachtrag erklärt die zitierte eigene Vorfassung als zu schwach; Fundstelle entstand in `af92adb` | — |
| 6 | `handel/kanal-rechtsmatrix.md:816` | `vorläufige[n] Beratungsergebnis` | γ | P: kein Treffer im Rohbeleg-Pool; die anders flektierte R17-Offenfassung ist nicht dieser Wortlaut | — |
| 7 | `handel/kanal-rechtsmatrix.md:877` | `das Berufungsgericht sieht es anders` | β | eigene Kastenformel, nicht als Gerichtsoriginal behauptet; S: Ursprung `af92adb` | — |
| 8 | `handel/kanal-rechtsmatrix.md:935` | `Der Beschluss hätte ohne diesen Satz denselben Tenor. Das ist keine Bewertung, sondern die Struktur der Entscheidung.` | ζ | R: eigener, ausdrücklich gestrichener Strukturschluss | — |
| 9 | `handel/kanal-rechtsmatrix.md:957` | `die IRREFÜHRUNGS-Achse (§ 5 UWG)` | ζ | R: frühere Normzuordnung, im Reparaturvermerk als unbelegt ausgewiesen | — |
| 10 | `handel/kanal-rechtsmatrix.md:999` | `verschiedene Anlagenkürzel, verschiedene Gegenseiten` | β | eigene verdichtete Charakterisierung, nicht Quellenwortlaut; S: Ursprung `af92adb` | — |
| 11 | `handel/kanal-rechtsmatrix.md:1021` | `das Berufungsgericht hält Briefwerbung dieser Art für zulässig` | β | ausdrücklich als überdehnte Lesart gesperrt; S: Ursprung `af92adb` | — |
| 12 | `handel/kanal-rechtsmatrix.md:1417` | `sonst existiert es im Prozess nicht` | ζ | R: frühere eigene Überspitzung, ausdrücklich repariert | — |
| 13 | `handel/kanal-rechtsmatrix.md:1418` | `Der Verstoß erfüllt keinen der speziellen Unlauterkeitstatbestände` | ζ | R: frühere Fehlableitung im Prüferbefund M-3 | — |
| 14 | `handel/kanal-rechtsmatrix.md:1418` | `der Auffangweg, nicht der erste Zugriff` | ζ | R: aus derselben früheren Fehlableitung gestrichen | — |
| 15 | `handel/kanal-rechtsmatrix.md:1419` | `die einzige, die im dortigen Fall glatt durchlief` | ζ | R: früherer Superlativ ohne benanntes Kriterium | — |
| 16 | `handel/kanal-rechtsmatrix.md:1420` | `dazu bestimmt sein, im Interesse der Marktteilnehmer das Marktverhalten zu regeln` | γ | P: fremder Entscheidungswortlaut, kein Treffer im vorliegenden Rohbeleg-Pool | — |
| 17 | `handel/kanal-rechtsmatrix.md:1421` | `sich nicht lediglich reflexartig zu Gunsten von Marktteilnehmern auswirken, sondern muss jedenfalls auch — dieser Zweck muss nicht der einzige und nicht einmal der primäre sein — die Funktion haben, die wettbewerblichen Belange … zu schützen, wobei das geschützte Interesse gerade durch die Marktteilnahme … berührt werden muss` | γ | P: fremder Entscheidungswortlaut, kein Treffer im vorliegenden Rohbeleg-Pool | — |
| 18 | `handel/kanal-rechtsmatrix.md:1422` | `normal informierte, angemessen aufmerksame und verständige Durchschnittsverbraucher` | γ | P: fremder Entscheidungswortlaut, kein Treffer im vorliegenden Rohbeleg-Pool | — |
| 19 | `handel/kanal-rechtsmatrix.md:1441` | `*… und sie ist die einzige, deren Tatbestand B2B ausdrücklich einschließt*` | ζ | R: ausdrücklich zitierte und widerlegte Erstfassung der Überschrift | — |
| 20 | `handel/kanal-rechtsmatrix.md:1455` | `Doppelbegründung § 3a UWG und § 3 Abs. 2 UWG` | β | eigene Bezeichnung; S mit `Darstellung „Doppelbegründung …`: Ursprung der Fundstelle `af92adb` | — |
| 21 | `handel/kanal-rechtsmatrix.md:1468` | `gilt … nur für Informationen, die im Rahmen der auf die Förderung des Produktabsatzes gerichteten gewerblichen Tätigkeit — also vor Abgabe der Vertragserklärung des Verbrauchers — zu erteilen sind` | γ | P: fremder Entscheidungswortlaut, kein Treffer im vorliegenden Rohbeleg-Pool | — |
| 22 | `handel/kanal-rechtsmatrix.md:1506` | `Die Grenze der Vermutungsregel` | ζ | R: frühere falsche Randnummern-Zuordnung im Reparaturkasten M-4 | — |
| 23 | `handel/kanal-rechtsmatrix.md:1509` | `die mit Antrag V angegriffene … Untätigkeit` | γ | P: fremder Entscheidungswortlaut, kein Treffer im vorliegenden Rohbeleg-Pool | — |
| 24 | `handel/kanal-rechtsmatrix.md:1539` | `Ein unzulässiges Verhalten begründet eine tatsächliche Vermutung für die Wiederholung identischer und kerngleicher Verstöße.` | γ | P: fremder Entscheidungswortlaut, kein Treffer im vorliegenden Rohbeleg-Pool | — |
| 25 | `handel/kanal-rechtsmatrix.md:1544` | `widerlegbar praktisch nur über eine strafbewehrte Unterlassungserklärung` | ζ | R: frühere eigene Rechtsaussage, ausdrücklich gestrichen | — |
| 26 | `handel/kanal-rechtsmatrix.md:1561` | `Beauftragte (Lettershop, Versanddienstleister, Freelancer) fallen darunter` | ζ | R: frühere Subsumtion ohne Vorbehalt, ausdrücklich repariert | — |
| 27 | `handel/kanal-rechtsmatrix.md:1569` | `das Landgericht hat der Klage stattgegeben` | β | eigene zitierte Warnformel gegen eine Vollstattgabe-Lesart; S mit `Klage statt`: Ursprung `af92adb` | — |
| 28 | `handel/angebotsarchitektur.md:257` | `*Sie ist damit die einzige der fünf Bedingungen, die nach der Übergabe fortwirkt — und die einzige, deren Verletzung wir nicht selbst verhindern können.*` | ζ | R: als „Vorfassung wörtlich“ im Reparaturkasten bezeichnet; Ursprung `af92adb` | — |
| 29 | `handel/angebotsarchitektur.md:261` | `der Kunde — und wir beim Aufsetzen — licensing, copyright, or other notices nicht entfernen oder verdecken` | ζ | R: Selbstzitat im selben Reparaturkasten; Ursprung der Fundstelle `af92adb` | — |
| 30 | `beleg/baseline-messplan.md:401` | `alle im Fenster anfallenden` | ε | B: derselbe Baseline-Text steht schon in `beleg/baseline-messplan.md:144`; der Matcher verbraucht den Baseline-Eintrag einmal | — |
| 31 | `beleg/baseline-messplan.md:430` | `findet H-1 repo-weit 92-mal` | ζ | R: ausdrücklich als nicht reproduzierbare Erstfassung der Zählung dokumentiert | — |
| 32 | `handel/anwalts-briefing-2026-08.md:27` | `und schließt nur SP-04 (Personenname) und das Grund-Feld aus` | ζ | R: zitierte, als falsch erklärte R16-A-Vorfassung | **A** |
| 33 | `handel/anwalts-briefing-2026-08.md:27` | `tragen nur SP-02, SP-03, SP-05 bis SP-08 und SP-10 — niemals SP-04` | ζ | R: zitierter Gegenwortlaut in derselben SP-Reparatur | **A** |
| 34 | `handel/anwalts-briefing-2026-08.md:630` | `…, dem unser damaliger Aufruf nicht folgte` | ζ | R: frühere, als nicht feststellbar ausgewiesene Ursachenbehauptung | — |
| 35 | `handel/preismodell-optionen.md:1315` | `Die Fälligkeit vor dem ERSTEN DATENSATZ … ruht auf einem einzigen Satz aus Rn. 87 … Dieser Satz steht nicht in der tragenden Absagebegründung. … Tragend ist Rn. 84/85: Die lit.-f-Voraussetzungen sind schon deshalb nicht erfüllt, weil die Beklagte die Artt. 12, 14, 21 DSGVO verletzt hat. … Rn. 87 beginnt mit Unabhängig davon erscheinen die Bedenken im Ausgangspunkt allenfalls … erwägenswert — es ist die zweite, hilfsweise Abwehr desselben Einwands, nicht der Absagegrund. … Was das ändert und was nicht: Der Satz ist echt, richtig zitiert und steht in einem veröffentlichten Urteil. Aber er trägt die Entscheidung nicht — und auf ihm ruht die gesamte Terminlage (F-7, O-9, die Bau-Pflichten, die Sperrung des ersten Datensatzes).` | β | als eigenes dokumentübergreifendes Bestandszitat bezeichnet, aber nicht byte-identisch in einer anderen Zieldatei; S mit `ERSTEN DATENSATZ`: Fundstelle `af92adb` | — |
| 36 | `handel/preismodell-optionen.md:1332` | `das Landgericht Düsseldorf verlangt das` | β | eigene Warnformel gegen eine überdehnte Kurzbegründung; S: Ursprung `af92adb` | — |

Für die sieben γ-Fälle bedeutet „kein Treffer“ ausschließlich: **Der geladene lokale Pool aus 696 Textdateien trägt den normalisierten Wortlaut nicht.** Daraus folgt keine Aussage, dass die fremde Quelle den Satz nicht enthält oder außerhalb dieses Pools nicht existiert.

## 7. `EIGEN_R18`: sieben Aufnahmen, ein begründeter Nicht-Eintrag

Die sieben in `protokolle/R17-A-abschluss.md:527–533` übergebenen Kandidaten wurden mit den dort getragenen Gründen und mit Fundstellenstand `86afceb` aufgenommen:

| Text | Grund in `EIGEN_R18` | Inventarklasse |
|---|---|:---:|
| `das Berufungsgericht sieht es anders` | eigene Kastenüberschrift | β |
| `das Berufungsgericht hält Briefwerbung dieser Art für zulässig` | bewusst gesperrte Lesart | β |
| `Doppelbegründung § 3a UWG und § 3 Abs. 2 UWG` | eigene Bezeichnung | β |
| `das LG stützt sich auf § 3a UWG` | bewusst gesperrte Lesart | β |
| `über § 3a UWG bzw. § 3 Abs. 2 UWG` | Selbstzitat der Vorfassung aus derselben Datei | ζ |
| `*Sie ist damit die einzige der fünf Bedingungen, die nach der Übergabe fortwirkt — und die einzige, deren Verletzung wir nicht selbst verhindern können.*` | Selbstzitat der eigenen Vorfassung im Reparaturkasten | ζ |
| `der Kunde — und wir beim Aufsetzen — licensing, copyright, or other notices nicht entfernen oder verdecken` | Selbstzitat aus derselben Datei | ζ |

Jeder Eintrag hat eine positive und eine abgewandelte negative Selbsttest-Fundstelle: `7/7 EIGEN`, `7/7` Abweichungen bleiben NEU. Alle übrigen β-Texte wurden **nicht** aufgenommen.

Der achte Kandidat aus `protokolle/R17-A-abschluss.md:535–542`, `„verbleiben“`, wurde bewusst **nicht** aufgenommen. Der reine Inhalt hat nur zehn Zeichen und unterschreitet die Extraktionsschwelle von 25 Zeichen; er erzeugt daher keine NEU-Fundstelle. Ein EIGEN-Eintrag wäre sofort verwaist. Die lange reale Rn.-154-Fundstelle ist bereits Baseline und benötigt keinen neuen EIGEN-Eintrag.

## 8. Produktiver Lauf nach der Reparatur

Der finale Lauf mit dem gelieferten Skript aus dem Live-Repo gegen den Export `86afceb` endete erwartungsgemäß mit Exit `1`, weil 29 inventarisierte, nicht freigegebene NEU verbleiben. Die Kontrollen selbst sind vollständig grün:

```text
Geprueft: 2123 Zitate (>=25 Zeichen, deutsche Anfuehrungszeichen)
Quellenzitate bestaetigt (versionierter Pool): 503
Bestaetigt (Rohbeleg, lokal): 556
Eigene Formulierungen: 35 / 35 Eintraege im Bestand vorhanden (35 als EIGEN klassifizierte Fundstellen)
EIGEN-Eintraege mit Begruendung: 35 / 35
R15-Altlast: 507 von 863 noch offen
R16-Aufnahmebestand der zwei neuen Zieldateien: 43 von 43 noch offen
R17-Aufnahmebestand der 19 neuen Zieldateien: 442 von 447 noch offen
Neu: 29
...
Die rote Gesamtampel ist damit kein Testfehler, sondern der korrekte Wächterzustand für den offenen Restnenner.

## 9. Tatsächliche Läufe aller Modi und Ausgabe-Archiv

Alle stdout-/stderr-Dateien und der Baseline-Entwurf liegen im Scratch-Verzeichnis außerhalb des Repos. Die Hashes archivieren die unveränderte Vollausgabe; stderr war in allen Läufen leer (`e3b0c442…`).

| Lauf / Modus | Exit | Ergebnis | vollständige stdout-Datei | SHA-256 |
|---|---:|---|---|---|
| ursprünglicher Hauptlauf | 1 | 36 NEU; PK 33/33; Gegenprobe 3/3; Blockzitate 3/3 | `lauf-0-original-haupt.txt` (18.951 Byte) | `a3fa8b0f879f1bd7c85c2a681ec339afe2f8603b5a21b7b70000ecfe1db57c27` |
| neuer Selbsttest vor Fix | 1 | beabsichtigte rote Regression | `lauf-1-vor-fix-selbsttest-rot.txt` (1.274 Byte) | `38b6a1fa1704f44f5a126c29baf422c20fda682a99e3461d14d1c2cd48496b62` |
| Selbsttest nach Fix | 0 | 46/46, bestanden | `lauf-7-delivery-selftest.txt` (1.267 Byte) | `ba453238456871edfd92c866a23f86346df202f8ac8018f71d3bd8172c36c942` |
| finaler Hauptlauf | 1 | 29 NEU, alle Kontrollen grün | `lauf-8-delivery-haupt.txt` (18.133 Byte) | `34114a15d60b63eba10911a79563a30e967d025d0a3ce53c0f9624f73c07e3f8` |
| `--altlast` | 0 | 1.353/1.353 Baseline-Einträge gelistet | `lauf-4-altlast.txt` (176.831 Byte) | `fc8458c20773d2ee0df45320ea0343dba3fd7bac33e8c3e8c6d1e1a6b63ffa6a` |
| `--zielkandidaten` | 0 | 27/27 ZIEL; einziger neuer Kandidat ist die Selbsttestdatei mit 46 Zitaten | `lauf-5-zielkandidaten.txt` (199 Byte) | `53d1f346beccf3cf12e73cf461a34d8206fe6d23c774c55e372c426261605cb1` |
| `--baseline-entwurf` | 1 | Entwurf geschrieben; Sollwertvergleich meldet erwartete Textstands-Drift | `lauf-6-baseline-entwurf.txt` (1.917 Byte) | `303bc342ec7d81cef2e5abb9ddc2a03ef8b162d10de466c02d89f01beec523a6` |
| erzeugter Baseline-Entwurf | — | nur Scratch, nicht übernommen | `baseline-entwurf-r18b.md` (136.046 Byte) | `5f88ac451c30e94e2f77f3257429b18d3bdb1446667fe3450b936abc0674a951` |

Der Modus `--baseline-entwurf` inventarisiert den **gegenwärtigen Textinhalt** neu und vergleicht ihn mit älteren Sollzahlen; am Export `86afceb` meldet er deshalb unter anderem 11 statt 9 in `beleg/baseline-messplan.md`. Dieser Exit `1` ist von der produktiven konsumierenden Baseline-Prüfung zu unterscheiden. Die bestehende Baseline-Datei wurde nicht ersetzt; deren produktive Nenner bleiben exakt.

### 9.1 Encoding- und Zeilenendenkontrolle

Beide geänderten Bestandsdateien wurden vor (`git show 86afceb:…`) und nach der Änderung mit einem fatalen UTF-8-Decoder gelesen; kein Decode-Fehler und kein U+FFFD-Ersatzzeichen. Beide blieben LF-Dateien (`CRLF = 0`). Die addierten Selbsttests erklären die erwartete Zunahme der Umlaute; es gab keinen Verlust:

| Zeichenklasse, beide Dateien zusammen | vor | nach |
|---|---:|---:|
| `ä / ö / ü` | `61 / 6 / 60` | `89 / 9 / 80` |
| `Ä / Ö / Ü / ß` | `0 / 0 / 4 / 11` | `0 / 0 / 7 / 13` |
| NBSP U+00A0 | 1 | 3 |
| geschützter Bindestrich U+2011 | 0 | 0 |
| Ersatzzeichen U+FFFD | 0 | 0 |

Die zwei zusätzlichen NBSP stehen als bewusstes Suchzeichen in `normLegacy()` und im expliziten Testhelfer für die falsche Stufenfolge. `git diff --check` und `node --check akquise/pruefe-zitate.js` liefen ohne Befund.

## 10. Entscheidungen für die Leitsession

R18-B inventarisiert und misst; folgende Policy-Entscheide bleiben ausdrücklich bei der Leitsession:

1. **δ (2):** Sollen Zieldateien generell als zitierfähige Quellen gelten, oder sollen nur diese beiden Gate-Sätze als eigene Formulierungen behandelt werden? Ein pauschales Hinzufügen aller Zieltexte zum POOL würde den bewussten Selbstausschluss unterlaufen.
2. **ε (1):** Soll die Baseline pro Text oder pro Vorkommen zählen? Der aktuelle Matcher konsumiert einen identischen Text einmal; die zweite reale Fundstelle wird dadurch NEU.
3. **ζ (18):** Für noch nicht ausdrücklich freigegebene Reparaturzitate empfiehlt sich eine eigene, klar benannte Konstante wie `REPARATURVERMERK_R18`, statt sie still als externe Quelle oder als allgemeines EIGEN zu behandeln. R18-B nimmt ohne Leitsessionsentscheid keine weiteren ζ-Fälle auf. Drei ζ-Fälle wurden nur deshalb in `EIGEN_R18` aufgenommen, weil der R18-B-Auftrag genau diese sieben Kandidaten ausdrücklich anwies.
4. **γ (7):** Die Quellbeschaffung bzw. eine Entscheidung über den Umgang mit lokal nicht getragenen Fremdzitaten bleibt offen; der Wächter darf aus einem fehlenden Pooltreffer keine materielle Falschheit ableiten.
5. **β (übrige 4):** Nicht aufgenommen. Ob auch diese Warn- und Selbstformulierungen zulässige EIGEN-Ausnahmen werden, entscheidet die Leitsession einzeln.

## 11. Nicht erhoben / nicht prüfbar

- Nicht erhoben wurde die materielle juristische Richtigkeit der 36 Texte.
- Nicht geprüft wurde, ob die sieben γ-Zitate in einer Quelle außerhalb des geladenen Rohbeleg-Pools stehen.
- Nicht gemessen wurde der wandernde Arbeitsbaum als Dokumentenstand; insbesondere flossen parallele Änderungen von Strang A nicht in ZIEL oder POOL ein.
- Nicht entschieden wurden die Policy-Fragen δ, ε und die noch offenen ζ-/β-Aufnahmen.
- Nicht geprüft wurde eine externe Webquelle; dieser Strang hatte einen lokalen, vorgegebenen Prüfpool.
- Der git-ignorierte Rohbeleg-Pool besitzt keinen Git-Prüfstand. Er war bei den Läufen stabil mit 696 Textdateien und 9.228.861 Byte sichtbar; darüber hinaus ist seine historische Reproduzierbarkeit nicht aus dem Commit `86afceb` ableitbar.

## 12. Welcher Prüfschritt hätte den vorhandenen Fehler NICHT finden können?

Die **33 Positivkontrollen**, die **drei normalen Gegenproben** und selbst die **drei fest verdrahteten Blockzitat-Paarungen** hätten den Fehler nicht finden können: Keine davon enthielt ein verschachteltes `> >` **mitten in einem extrahierten deutschen Anführungszitat**, einmal nur auf Ziel- und einmal nur auf Quellseite. Sie prüften bekannte Inhalte, nicht diese Normalisierungsachse. Auch der reine Nennervergleich hätte den Fehler nur als NEU-Symptom gezeigt, nicht seine Ursache. Erst die absichtlich vorher rot ausgeführte, isolierte Muster-J-Regression trennt Präfixentfernung, Symmetrie und Dehyphenierungsreihenfolge.

## 13. Arbeitsbaum- und Parallelhinweis

Die vier R18-B-Scope-Dateien waren vor der Bearbeitung sauber. Während der Arbeit erschienen fremde Änderungen in `akquise/interessenabwaegung-o8.md`, `akquise/listenbau-regelwerk.md`, `akquise/sperrdatei-struktur.md`, `handel/angebotsarchitektur.md`, `handel/anwalts-briefing-2026-08.md` und `handel/kanal-rechtsmatrix.md`; sie wurden von R18-B weder verändert noch als Live-Arbeitsbaum gemessen. Im letzten Status-Snapshot waren diese Paralleländerungen nicht mehr uncommitted sichtbar; der geteilte Branch stand auf `main...origin/main [ahead 1]` und zeigte nur `M akquise/pruefe-zitate.js`, `M akquise/selbsttest-zitate.md` sowie `?? protokolle/R18-B-bericht.md`. Sowohl der vorausliegende Commit als auch zwischenzeitliche Statusänderungen sind fremder Parallelkontext. Ein abschließendes `git status` ist deshalb nur eine Momentaufnahme und kein Eigentumsnachweis. R18-B beansprucht ausschließlich die drei oben genannten Änderungen; es wurde nichts committed oder gepusht.

## 14. Erfüllter Status-Punkt

Erfüllt ist der R18-Wächter-Arbeitsanteil „36 NEU inventarisieren, verschachtelte `> >`-Normalisierung regressionsgesichert reparieren und die sieben freigegebenen Eigenformulierungen aufnehmen“. Die Bewertung der 29 verbleibenden Fundstellen und die Policy-Entscheide bleiben bewusst offen.

## Anhang — vollständige Rohausgaben

Der grüne Selbsttest ist bereits vollständig in § 4 wiedergegeben. Es folgen die übrigen stdout-Ausgaben ohne Kürzung; sämtliche stderr-Dateien waren leer.

### A — finaler Hauptlauf (`lauf-8-delivery-haupt.txt`)
...
ZIEL: 27 / 27 Dateien | versionierter POOL: 12 Dateien
Rohbeleg-Pool: verfuegbar (1 Wurzeln, 696 Textdateien, 9228861 Bytes; Argument --rohbelege)
Geprueft: 2123 Zitate (>=25 Zeichen, deutsche Anfuehrungszeichen)
Quellenzitate bestaetigt (versionierter Pool): 503
Bestaetigt (Rohbeleg, lokal): 556
Eigene Formulierungen: 35 / 35 Eintraege im Bestand vorhanden (35 als EIGEN klassifizierte Fundstellen)
...
akquise/akquiseplan.md: 48 geprueft | 42 bestaetigt (Pool) | 0 bestaetigt (Rohbeleg, lokal) | 4 eigene | 0 Bestand (-) | 0 nicht pruefbar (Rohbeleg-Pool nicht verfuegbar) | 0 Rohbeleg bestaetigt nicht (R17-Aufnahmebefund) | 2 NEU
akquise/listenbau-regelwerk.md: 127 geprueft | 116 bestaetigt (Pool) | 0 bestaetigt (Rohbeleg, lokal) | 11 eigene | 0 Bestand (-) | 0 nicht pruefbar (Rohbeleg-Pool nicht verfuegbar) | 0 Rohbeleg bestaetigt nicht (R17-Aufnahmebefund) | 0 NEU
akquise/interessenabwaegung-o8.md: 160 geprueft | 41 bestaetigt (Pool) | 53 bestaetigt (Rohbeleg, lokal) | 2 eigene | 63 Bestand (r15-altlast=63) | 0 nicht pruefbar (Rohbeleg-Pool nicht verfuegbar) | 0 Rohbeleg bestaetigt nicht (R17-Aufnahmebefund) | 1 NEU
fund/wettbewerbsbild.md: 494 geprueft | 35 bestaetigt (Pool) | 239 bestaetigt (Rohbeleg, lokal) | 0 eigene | 220 Bestand (r15-altlast=220) | 0 nicht pruefbar (Rohbeleg-Pool nicht verfuegbar) | 0 Rohbeleg bestaetigt nicht (R17-Aufnahmebefund) | 0 NEU
handel/kanal-rechtsmatrix.md: 305 geprueft | 38 bestaetigt (Pool) | 138 bestaetigt (Rohbeleg, lokal) | 15 eigene | 87 Bestand (r15-altlast=87) | 0 nicht pruefbar (Rohbeleg-Pool nicht verfuegbar) | 8 Rohbeleg bestaetigt nicht (R17-Aufnahmebefund) | 19 NEU
handel/angebotsarchitektur.md: 174 geprueft | 35 bestaetigt (Pool) | 0 bestaetigt (Rohbeleg, lokal) | 2 eigene | 137 Bestand (r15-altlast=137) | 0 nicht pruefbar (Rohbeleg-Pool nicht verfuegbar) | 0 Rohbeleg bestaetigt nicht (R17-Aufnahmebefund) | 0 NEU
...
beleg/baseline-messplan.md: 16 geprueft | 5 bestaetigt (Pool) | 0 bestaetigt (Rohbeleg, lokal) | 0 eigene | 9 Bestand (r17-zielaufnahme=9) | 0 nicht pruefbar (Rohbeleg-Pool nicht verfuegbar) | 0 Rohbeleg bestaetigt nicht (R17-Aufnahmebefund) | 2 NEU
...
fund/positionierungspapier.md: 30 geprueft | 7 bestaetigt (Pool) | 0 bestaetigt (Rohbeleg, lokal) | 0 eigene | 23 Bestand (r17-zielaufnahme=23) | 0 nicht pruefbar (Rohbeleg-Pool nicht verfuegbar) | 0 Rohbeleg bestaetigt nicht (R17-Aufnahmebefund) | 0 NEU
handel/anwalts-briefing-2026-08.md: 148 geprueft | 68 bestaetigt (Pool) | 42 bestaetigt (Rohbeleg, lokal) | 0 eigene | 35 Bestand (r17-zielaufnahme=35) | 0 nicht pruefbar (Rohbeleg-Pool nicht verfuegbar) | 0 Rohbeleg bestaetigt nicht (R17-Aufnahmebefund) | 3 NEU
...
handel/entwurf-anfrage-wtsh.md: 14 geprueft | 3 bestaetigt (Pool) | 4 bestaetigt (Rohbeleg, lokal) | 0 eigene | 7 Bestand (r17-zielaufnahme=7) | 0 nicht pruefbar (Rohbeleg-Pool nicht verfuegbar) | 0 Rohbeleg bestaetigt nicht (R17-Aufnahmebefund) | 0 NEU
...
handel/preisliste-vorlaeufig.md: 79 geprueft | 28 bestaetigt (Pool) | 1 bestaetigt (Rohbeleg, lokal) | 0 eigene | 50 Bestand (r17-zielaufnahme=50) | 0 nicht pruefbar (Rohbeleg-Pool nicht verfuegbar) | 0 Rohbeleg bestaetigt nicht (R17-Aufnahmebefund) | 0 NEU
handel/preismodell-optionen.md: 142 geprueft | 16 bestaetigt (Pool) | 12 bestaetigt (Rohbeleg, lokal) | 0 eigene | 112 Bestand (r17-zielaufnahme=112) | 0 nicht pruefbar (Rohbeleg-Pool nicht verfuegbar) | 0 Rohbeleg bestaetigt nicht (R17-Aufnahmebefund) | 2 NEU

--- Bestand mit Nenner ---
R15-Altlast: 507 von 863 noch offen | 356 seit Baseline/Einbezug abgebaut | davon Rohbeleg-bestaetigt: 346
  akquise/interessenabwaegung-o8.md: 63 von 113 noch offen | davon Rohbeleg-bestaetigt: 50
  fund/wettbewerbsbild.md: 220 von 467 noch offen | davon Rohbeleg-bestaetigt: 239
  handel/kanal-rechtsmatrix.md: 87 von 144 noch offen | davon Rohbeleg-bestaetigt: 57
  handel/angebotsarchitektur.md: 137 von 139 noch offen | davon Rohbeleg-bestaetigt: 0
...
R17-Aufnahmebestand der 19 neuen Zieldateien: 442 von 447 noch offen | 5 seit Baseline/Einbezug abgebaut | davon Rohbeleg-bestaetigt: 0
  ...
  fund/positionierungspapier.md: 23 von 24 noch offen | davon Rohbeleg-bestaetigt: 0
  handel/anwalts-briefing-2026-08.md: 35 von 36 noch offen | davon Rohbeleg-bestaetigt: 0
  handel/entwurf-anfrage-ifb-hamburg.md: 4 von 4 noch offen | davon Rohbeleg-bestaetigt: 0
  handel/entwurf-anfrage-wtsh.md: 7 von 8 noch offen | davon Rohbeleg-bestaetigt: 0
  handel/foerderarchitektur-beraterrolle.md: 54 von 54 noch offen | davon Rohbeleg-bestaetigt: 0
  handel/preisliste-vorlaeufig.md: 50 von 51 noch offen | davon Rohbeleg-bestaetigt: 0
  handel/preismodell-optionen.md: 112 von 113 noch offen | davon Rohbeleg-bestaetigt: 0
...
[handel/kanal-rechtsmatrix.md:1335] je Schiene getrennt … und die Antworten zeigen in entgegengesetzte Richtungen

--- NEUE Verstoesse ---
Neu: 29
[akquise/akquiseplan.md:413] diese Datei ist noch NICHT scharf zu stellen
[akquise/akquiseplan.md:413] es darf noch keine Welle geben
[akquise/interessenabwaegung-o8.md:676] und sie ist die einzige, deren Tatbestand B2B einschließt
[handel/kanal-rechtsmatrix.md:816] vorläufige[n] Beratungsergebnis
[handel/kanal-rechtsmatrix.md:935] Der Beschluss hätte ohne diesen Satz denselben Tenor. Das ist keine Bewertung, sondern die Struktur der Entscheidung.
[handel/kanal-rechtsmatrix.md:957] die IRREFÜHRUNGS-Achse (§ 5 UWG)
[handel/kanal-rechtsmatrix.md:999] verschiedene Anlagenkürzel, verschiedene Gegenseiten
[handel/kanal-rechtsmatrix.md:1417] sonst existiert es im Prozess nicht
[handel/kanal-rechtsmatrix.md:1418] Der Verstoß erfüllt keinen der speziellen Unlauterkeitstatbestände
[handel/kanal-rechtsmatrix.md:1418] der Auffangweg, nicht der erste Zugriff
[handel/kanal-rechtsmatrix.md:1419] die einzige, die im dortigen Fall glatt durchlief
[handel/kanal-rechtsmatrix.md:1420] dazu bestimmt sein, im Interesse der Marktteilnehmer das Marktverhalten zu regeln
[handel/kanal-rechtsmatrix.md:1421] sich nicht lediglich reflexartig zu Gunsten von Marktteilnehmern auswirken, sondern muss jedenfalls auch — dieser Zweck muss nicht der einzige und nicht einmal der primäre sein — die Funktion haben, die wettbewerblichen Belange … zu schützen, wobei das geschützte Interesse gerade durch die Marktteilnahme … berührt werden muss
[handel/kanal-rechtsmatrix.md:1422] normal informierte, angemessen aufmerksame und verständige Durchschnittsverbraucher
[handel/kanal-rechtsmatrix.md:1441] *… und sie ist die einzige, deren Tatbestand B2B ausdrücklich einschließt*
[handel/kanal-rechtsmatrix.md:1468] gilt … nur für Informationen, die im Rahmen der auf die Förderung des Produktabsatzes gerichteten gewerblichen Tätigkeit — also vor Abgabe der Vertragserklärung des Verbrauchers — zu erteilen sind
[handel/kanal-rechtsmatrix.md:1506] Die Grenze der Vermutungsregel
[handel/kanal-rechtsmatrix.md:1509] die mit Antrag V angegriffene … Untätigkeit
[handel/kanal-rechtsmatrix.md:1539] Ein unzulässiges Verhalten begründet eine tatsächliche Vermutung für die Wiederholung identischer und kerngleicher Verstöße.
[handel/kanal-rechtsmatrix.md:1544] widerlegbar praktisch nur über eine strafbewehrte Unterlassungserklärung
[handel/kanal-rechtsmatrix.md:1561] Beauftragte (Lettershop, Versanddienstleister, Freelancer) fallen darunter
[handel/kanal-rechtsmatrix.md:1569] das Landgericht hat der Klage stattgegeben
[beleg/baseline-messplan.md:401] alle im Fenster anfallenden
[beleg/baseline-messplan.md:430] findet `H-1` repo-weit 92-mal
[handel/anwalts-briefing-2026-08.md:27] und schließt nur SP-04 (Personenname) und das Grund-Feld aus
[handel/anwalts-briefing-2026-08.md:27] tragen nur SP-02, SP-03, SP-05 bis SP-08 und SP-10 — niemals SP-04
[handel/anwalts-briefing-2026-08.md:630] …, dem unser damaliger Aufruf nicht folgte
[handel/preismodell-optionen.md:1315] Die Fälligkeit vor dem ERSTEN DATENSATZ … ruht auf einem einzigen Satz aus Rn. 87 … Dieser Satz steht nicht in der tragenden Absagebegründung. … Tragend ist Rn. 84/85: Die lit.-f-Voraussetzungen sind schon deshalb nicht erfüllt, weil die Beklagte die Artt. 12, 14, 21 DSGVO verletzt hat. … Rn. 87 beginnt mit Unabhängig davon erscheinen die Bedenken im Ausgangspunkt allenfalls … erwägenswert — es ist die zweite, hilfsweise Abwehr desselben Einwands, nicht der Absagegrund. … Was das ändert und was nicht: Der Satz ist echt, richtig zitiert und steht in einem veröffentlichten Urteil. Aber er trägt die Entscheidung nicht — und auf ihm ruht die gesamte Terminlage (F-7, O-9, die Bau-Pflichten, die Sperrung des ersten Datensatzes).
[handel/preismodell-optionen.md:1332] das Landgericht Düsseldorf verlangt das
...
--- R15-Altlast ---
Offen: 507 von 863 | seit Baseline/Einbezug abgebaut: 356 | davon Rohbeleg-bestaetigt: 346
...
fund/wettbewerbsbild.md: 220 von 467 offen | 247 abgebaut | davon Rohbeleg-bestaetigt: 239
[OFFEN] fund/wettbewerbsbild.md :: Stand Juli 2026, nicht neu bestätigt
[ABGEBAUT] fund/wettbewerbsbild.md :: Wer baut dieselbe Technik?
...
[ABGEBAUT] fund/wettbewerbsbild.md :: Teil A inhaltlich unverändert
[ABGEBAUT] fund/wettbewerbsbild.md :: Wer baut dieselbe Technik?
[OFFEN] fund/wettbewerbsbild.md :: Wer verkauft demselben Hotelier dieselbe Leistung?
...
[ABGEBAUT] fund/wettbewerbsbild.md :: ~70 Partner in sechs Kategorien
...
[ABGEBAUT] fund/wettbewerbsbild.md :: so ist der Partnerkreis gegliedert
...
handel/angebotsarchitektur.md: 137 von 139 offen | 2 abgebaut | davon Rohbeleg-bestaetigt: 0
...
[ABGEBAUT] handel/angebotsarchitektur.md :: verstößt gegen die Auflage
...
[ABGEBAUT] handel/angebotsarchitektur.md :: Das ist eine Feststellung des Bestands, keine neue Sperre dieser Session
...
Offen: 442 von 447 | seit Baseline/Einbezug abgebaut: 5 | davon Rohbeleg-bestaetigt: 0
...
fund/positionierungspapier.md: 23 von 24 offen | 1 abgebaut | davon Rohbeleg-bestaetigt: 0
...
[ABGEBAUT] fund/positionierungspapier.md :: heyPensio zu flach dargestellt
...
handel/anwalts-briefing-2026-08.md: 35 von 36 offen | 1 abgebaut | davon Rohbeleg-bestaetigt: 0
...
[ABGEBAUT] handel/anwalts-briefing-2026-08.md :: das LG stützt sich auf § 3a UWG
...
handel/entwurf-anfrage-wtsh.md: 7 von 8 offen | 1 abgebaut | davon Rohbeleg-bestaetigt: 0
...
[OFFEN] handel/entwurf-anfrage-wtsh.md :: nicht förderfähige Ausgaben
[ABGEBAUT] handel/entwurf-anfrage-wtsh.md :: Es gibt derzeit keinen konkreten Fall und keinen erteilten Auftrag
...
handel/preisliste-vorlaeufig.md: 50 von 51 offen | 1 abgebaut | davon Rohbeleg-bestaetigt: 0
...
[ABGEBAUT] handel/preisliste-vorlaeufig.md :: in der Fassung mit den ✅-Nachträgen
...
handel/preismodell-optionen.md: 112 von 113 offen | 1 abgebaut | davon Rohbeleg-bestaetigt: 0
...
[ABGEBAUT] handel/preismodell-optionen.md :: kein Versäumnis dieser Runde: Die Erhebung ist nie beauftragt worden.
...
### C — Zielkandidaten (`lauf-5-zielkandidaten.txt`)

~~~text
=== ZIEL-Kandidaten (generischer .md-Scan) ===
Suchraum: akquise/, handel/, fund/, beleg/
ZIEL: 27 / 27 Dateien
akquise/selbsttest-zitate.md: 46 Zitate
Kandidaten: 1 Dateien
Zitate in Kandidaten: 46

~~~

### D — Baseline-Entwurf (stdout) (`lauf-6-baseline-entwurf.txt`)

~~~text
Baseline-Entwurf geschrieben: C:\Users\qwafa_2dwxzia\firma\r18b-scratch-86afceb\baseline-entwurf-r18b.md
r15-altlast | akquise/interessenabwaegung-o8.md: 113 (Soll 113)
r15-altlast | fund/wettbewerbsbild.md: 467 (Soll 467)
r15-altlast | handel/kanal-rechtsmatrix.md: 144 (Soll 144)
r15-altlast | handel/angebotsarchitektur.md: 139 (Soll 139)
r16-zielaufnahme | akquise/sperrdatei-struktur.md: 19 (Soll 19)
r16-zielaufnahme | akquise/wellenprotokoll-vorlage.md: 24 (Soll 24)
r17-zielaufnahme | beleg/baseline-messplan.md: 11 (Soll 9)
r17-zielaufnahme | beleg/vorlagen/interviewleitfaden-baseline.md: 38 (Soll 38)
r17-zielaufnahme | beleg/vorlagen/referenzvereinbarung-pilotobjekte.md: 12 (Soll 12)
r17-zielaufnahme | beleg/vorlagen/tagesblatt-firzlaffs.md: 3 (Soll 3)
r17-zielaufnahme | beleg/vorlagen/tagesblatt-hostel-boninstrasse.md: 0 (Soll 0)
r17-zielaufnahme | fund/erhebung/brandnamic-partner-vollerhebung.md: 38 (Soll 38)
r17-zielaufnahme | fund/erhebung/entwurf-anfrage-dehoga-sh.md: 2 (Soll 2)
r17-zielaufnahme | fund/erhebung/entwurf-anfrage-ihk-sh.md: 1 (Soll 1)
r17-zielaufnahme | fund/erhebung/regionalstatistik-groessenklassen.md: 18 (Soll 18)
r17-zielaufnahme | fund/erhebung/strang2-strukturdaten.md: 17 (Soll 17)
r17-zielaufnahme | fund/erhebung/verbund-vollerhebung.md: 12 (Soll 12)
r17-zielaufnahme | fund/persona-validierungsplan.md: 7 (Soll 7)
r17-zielaufnahme | fund/positionierungspapier.md: 23 (Soll 24)
r17-zielaufnahme | handel/anwalts-briefing-2026-08.md: 37 (Soll 36)
r17-zielaufnahme | handel/entwurf-anfrage-ifb-hamburg.md: 4 (Soll 4)
r17-zielaufnahme | handel/entwurf-anfrage-wtsh.md: 7 (Soll 8)
r17-zielaufnahme | handel/foerderarchitektur-beraterrolle.md: 54 (Soll 54)
r17-zielaufnahme | handel/preisliste-vorlaeufig.md: 50 (Soll 51)
r17-zielaufnahme | handel/preismodell-optionen.md: 114 (Soll 113)
Nutzdaten-SHA-256: 5934bf7a387e050c00f9d357f7ca2d1e469e551a8269de5dfe95326b3b424426

~~~

### E — rote Regression vor dem Fix (`lauf-1-vor-fix-selbsttest-rot.txt`)
...
Muster (i) Auslassung bleibt NEU : 2 / 2
Muster (i) keine Dateigrenze     : JA
Muster (j) > > symmetrisch       : 0 / 2 bestaetigt
Muster (j) TREFFEN-NICHT         : 1 / 1 NEU
Praefix vor Dehyphenierung       : NEIN
Dehyphenierung davor verworfen   : JA
Muster (k) EIGEN_R18 positiv     : 7 / 7
Muster (l) EIGEN_R18 negativ     : 7 / 7 NEU
Muster (m) EIGEN_R17 Praefix     : 0 / 1 EIGEN
Muster (m) Gegenrichtung         : 1 / 1 NEU
Selbstausschluss ZIEL            : 27 / 27
LF/CRLF invariant                : JA
